...
> [We review policies against a set of key criteria by drawing on UK-specific evidence primarily as well as that from other countries. Outputs include a longlist appraisal scoring matrix, 'modelling blueprints' and recommendations for policy design and implementation.]{style="color: SteelBlue;"}

## Scope

### Focus

While the term 'circular economy' (CE) has only appeared more recently in policy documents released by UK public bodies, policies whose objectives are congruent with the CE concept by delivering against one of its 'value drivers' (see ['leveraging CE value using policy'](https://policylab.quarto.pub/policy-tools-for-a-uk-circular-economy/value-drivers.html)), have been introduced in the UK since at least the 1970s.
...
3.  **Recommendations** for policy design and implementation, to effectively and efficiently leverage instruments.
...
![MacFeely (2011)](images/Balanced-Supply-Use-Framework_W640.jpg){width="400"}
...
1.  likely **penetration rates** - for instance, a voluntary approach could be expected to have a lesser participation rate than a mandatory approach, those even mandatory approaches may have freeriders where enforcement is low;

2.  likely **scale of effect** per participant[^about-5];

3.  the potential **immediacy** of an instrument's effects following introduction;

4.  potential **costs** and long-run effects such as innovation that may serve to reduce these; and

5.  **spillover** effects.

[^about-5]: Assuming full compliance, very direct with prescriptive regulation which effectively prescribes the outcome, though this will be parameterized by what is technologically feasible in many cases (apart from if technology-forcing policies are sought to be introduced). More indirect policy interventions which don't prescribe outcomes e.g. labelling, EPR or fiscal mechanisms, requires informed assumptions regarding likely efficacy of impact depending on intermediating parameter

These evidence-derived assumptions can be used alongside additional hypothetical policy assumptions including:

1.  The **scope** of a lever e.g. whether a policy is applied to computers or all small appliances and institutional scope - i.e. to **whom** the lever would apply and any exemptions;

2.  The **focus** of the instrument e.g. reducing production leakage, lightweighting and material substitution or lifespan extension;

3.  At what **level** the lever is set e.g. the length of the mandatory minimum lifetime of product or rate of tax applied; and

4.  **When** the lever is introduced, including accounting for sequencing and interaction effects

to input to the modelling of policy impacts in a consistent way. We outline the input requirements of core appraisal methodologies below. These approaches are increasingly being used in conjunction with one another to answer stakeholder questions relating to the circular economy ([Hellweg *et al.* 2023](https://doi.org/10.1038/s43017-023-00449-2)).
...
Aguilla-Hernandez et al. [(2018)](https://eur03.safelinks.protection.outlook.com/?url=https%3A%2F%2Fjournalofeconomicstructures.springeropen.com%2Farticles%2F10.1186%2Fs40008-018-0113-3&data=04%7C01%7Coliver.lysaght%40defra.gov.uk%7C411bb775c63e4244f04b08d91475ef64%7C770a245002274c6290c74e38537f1102%7C1%7C0%7C637563320077216388%7CUnknown%7CTWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAiLCJQIjoiV2luMzIiLCJBTiI6Ik1haWwiLCJXVCI6Mn0%3D%7C1000&sdata=z%2FDMKW1Zi3TB%2BTtdYsgXe8TXf2saIxC%2FQimmWhHy44c%3D&reserved=0) in a review of publications analysing interventions associated with a more circular economy through Environmentally-Extended Input-Output Modelling (EE-IOMs), summarised:
...
[^about-6]: Technological change consists of innovation of new technologies that were not available thereby expanding the production-possibility frontier, and diffusion of existing technologies to new users, thereby reducing the gap between the average production techniques used and the frontier.

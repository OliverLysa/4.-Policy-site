...
url <- "https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/913177/WEEE_received_at_an_approved_authorised_treatment_facility.ods"

# Specify destination where file should be saved
destfile <- "/Users/oliverlysaght/Desktop/R/WPP/WEEE/EA/WEEE_received_AATF.ods"
...
# 2020
EEE20 <-
  read_ods("WEEE_received_AATF.ods", sheet = 1) %>% as.data.frame()
...
# 2019
EEE19 <-
  read_ods("WEEE_received_AATF.ods", sheet = 2) %>% as.data.frame()
...
# 2018
EEE18 <-
  read_ods("WEEE_received_AATF.ods", sheet = 3) %>% as.data.frame()
...
# 2017
EEE17 <-
  read_ods("WEEE_received_AATF.ods", sheet = 4) %>% as.data.frame()
...
# 2016
EEE16 <-
  read_ods("WEEE_received_AATF.ods", sheet = 5) %>% as.data.frame()
...
# 2015
EEE15 <-
  read_ods("WEEE_received_AATF.ods", sheet = 6) %>% as.data.frame()
...
# 2014
EEE14 <-
  read_ods("WEEE_received_AATF.ods", sheet = 7) %>% as.data.frame()
...
# 2013
EEE13 <-
  read_ods("WEEE_received_AATF.ods", sheet = 8) %>% as.data.frame()
...
# 2012
EEE12 <-
  read_ods("WEEE_received_AATF.ods", sheet = 9) %>% as.data.frame()
...
# 2011
EEE11 <-
  read_ods("WEEE_received_AATF.ods", sheet = 10) %>% as.data.frame()
...
# 2010
EEE10 <-
  read_ods("WEEE_received_AATF.ods", sheet = 11) %>% as.data.frame()
...
# 2009
EEE09 <-
  read_ods("WEEE_received_AATF.ods", sheet = 12) %>% as.data.frame()
...
# 2008
EEE08 <-
  read_ods("WEEE_received_AATF.ods", sheet = 13) %>% as.data.frame()
...
receivedtotal <- receivedtotal %>% mutate(Received_Type = "AATF")

# WEEE received non-obligated

# Specify URL where file is stored
url <- "https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/913182/Non-obligated_WEEE_received_at_approved_authorised_treatment_facilities_and_approved_exporters.ods"

# Specify destination where file should be saved
destfile <- "/Users/oliverlysaght/Desktop/R/WPP/WEEE/EA/non_obligated_received.ods"
...
#Bind both received datasets
receivedall <- rbind(receivedtotal, receivednonob)
